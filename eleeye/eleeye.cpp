...
#include "logger.h"
  ...
  #include <dlfcn.h>
  #define WINAPI
  const char *const cszLibEvalFile = "engine/libeval.so";
#endif
...
int main(void) {
  ...
  LocatePath(Search.szBookFile, "engine/BOOK.DAT");
  ...
  PreGenInit();
  NewHash(24); // 24=16MB, 25=32MB, 26=64MB, ..., 这个是创建 置换表的过程.
  // fen值
  ...
  while (!Search.bQuit) {
    ...
    case UCCI_COMM_SETOPTION:
      ...
      case UCCI_OPTION_PROMOTION:
        ...
        break;
      case UCCI_OPTION_LOG:
        getLogger()->init(UcciComm.szOption);
    ...
    case UCCI_COMM_GO:
      ...
      case UCCI_GO_DEPTH:
        ...
        Search.nNodes = 0;
        SearchMainTimer(UcciComm.nDepth);
        break;
      case UCCI_GO_NODES:
        Search.nGoMode = GO_MODE_NODES;
        Search.nNodes = UcciComm.nNodes;
        SearchMainTimer(UCCI_MAX_DEPTH);
        break;
