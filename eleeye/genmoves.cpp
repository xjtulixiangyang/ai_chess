...
int PositionStruct::GenNonCapMoves(MoveStruct *lpmvs) const {
  ...
  END_TIME(NOCAPMOVE_INDEX)
  return lpmvsCurr - lpmvs;
}

// 添加计时器“捉”的检测
int PositionStruct::ChasedByTimer(int mv) const{
    START_TIME(CHASED_BY_INDEX)
    int ret = ChasedBy(mv);
    END_TIME(CHASED_BY_INDEX)
    return ret;
}
