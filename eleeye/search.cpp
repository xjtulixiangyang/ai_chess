...
void BuildPos(PositionStruct &pos, const UcciCommStruct &UcciComm) {
  ...
  for (i = 0; i < UcciComm.nMoveNum; i ++) {
    ...
    bool suc = pos.MakeMove(mv);

    if(! suc ){
	// 默认当前步骤数没有到极限.
	if ( pos.sdPlayer != (UcciComm.nMoveNum + orgSd) % 2 )
	{
	    printf("win\n");
	}
    }
...
static inline int SearchQuiescTimer(PositionStruct &pos, int vlAlpha, int vlBeta) {
    START_TIME(SEARCHQUIESC_INDEX)
    int result = SearchQuiesc(pos, vlAlpha, vlBeta);
    END_TIME(SEARCHQUIESC_INDEX)
    return result;
}
...
void PopLeaf(PositionStruct &pos) {
  int vl;
  Search2.nAllNodes = 0;
  vl = SearchQuiescTimer(pos, -MATE_VALUE, MATE_VALUE);
...
static int SearchCut(int vlBeta, int nDepth, bool bNoNull = false) {
  ...
  if (nDepth <= 0) {
    ...
    return SearchQuiescTimer(Search.pos, vlBeta - 1, vlBeta);
  }
  Search2.nAllNodes ++;
...
static int SearchPV(int vlAlpha, int vlBeta, int nDepth, uint16_t *lpwmvPvLine) {
  ...
  *lpwmvPvLine = 0;
  if (nDepth <= 0) {
    __ASSERT(nDepth >= -NULL_DEPTH);
    return SearchQuiescTimer(Search.pos, vlAlpha, vlBeta);
  }
  Search2.nAllNodes ++;
...
void SearchMain(int nDepth) {
  printf("cur turn %s\n", (Search.pos.sdPlayer == 0 ? "red" : "black"   ));
  ...
  // 3. 如果深度为零则返回静态搜索值
  if (nDepth == 0) {
    printf("info depth 0 score %d\n", SearchQuiescTimer(Search.pos, -MATE_VALUE, MATE_VALUE));
    fflush(stdout);
    printf("nobestmove\n");
    fflush(stdout);
    return;
...
void SearchMainTimer(int depth){
    // 输出上一次的time
    RESET_TIME()

    //介绍起始棋局状况.
    getLogger()->logger("cur turn %s, move num %d\n", 
            (Search.pos.sdPlayer == 0 ? "red" : "black" ), 
            Search.pos.nMoveNum );

    START_TIME(SEARCH_INDEX)
    SearchMain(depth);
    END_TIME(SEARCH_INDEX)
